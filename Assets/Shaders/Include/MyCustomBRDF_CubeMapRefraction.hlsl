...
// IOR values for chromatic dispersion
    half ior_r = 1.2;
    half ior_g = 1.21;
    half ior_b = 1.22;

// First refraction: entering front surface (air → material)
    half3 refractIn_R = refract(-viewDirWS, inputData.normalWS, 1.0 / ior_r);
    half3 refractIn_G = refract(-viewDirWS, inputData.normalWS, 1.0 / ior_g);
    half3 refractIn_B = refract(-viewDirWS, inputData.normalWS, 1.0 / ior_b);

// Back surface normal (thin slab assumption: parallel surfaces)
    half3 backNormal = -inputData.normalWS;

// Second refraction: exiting back surface (material → air)
    half3 refractOut_R = refract(refractIn_R, backNormal, ior_r / 1.0);
    half3 refractOut_G = refract(refractIn_G, backNormal, ior_g / 1.0);
    half3 refractOut_B = refract(refractIn_B, backNormal, ior_b / 1.0);

// Fallback for total internal reflection
    refractOut_R = length(refractOut_R) > 0.001 ? refractOut_R : refractIn_R;
    refractOut_G = length(refractOut_G) > 0.001 ? refractOut_G : refractIn_G;
    refractOut_B = length(refractOut_B) > 0.001 ? refractOut_B : refractIn_B;

// Sample cubemap with double-refracted directions
    half indirectSpecularR = MyGISpecular(refractIn_R, brdfData.perceptualRoughness, occlusion).r;
    half indirectSpecularG = MyGISpecular(refractIn_G, brdfData.perceptualRoughness, occlusion).g;
    half indirectSpecularB = MyGISpecular(refractIn_B, brdfData.perceptualRoughness, occlusion).b;
    
    //-------------------------------------------------------------------------
    // COMBINE using environment BRDF
    //-------------------------------------------------------------------------
    return MyEnvironmentBRDF(brdfData, indirectDiffuse, float3(indirectSpecularR, indirectSpecularG, indirectSpecularB), NdotV);
}
